...
def draw_figure(frame: pd.DataFrame, path: Path, section: str, title: str) -> None:
...
def specification_rows(frame: pd.DataFrame, populations: pd.DataFrame, section: str) -> list[list[str]]:
    ...
    for row_key in ordered_unique(frame["row_key"]):
        record = frame[frame["row_key"].eq(row_key)].iloc[0]
        changes.append(
            f"{record['row_label']}: {record['constructor']}, {record['pattern'].replace('_', ' ')} changes in P and Q"
        )
...
def null_rows(frame: pd.DataFrame) -> list[list[str]]:
    ...
    rows = []
    for record in null.sort_values(["row_key", "column_key", "method"]).itertuples():
        rows.append([
            record.row_label, record.column_label, record.method_label,
...
def detail_rows(frame: pd.DataFrame, section: str) -> list[list[str]]:
    rows = []
    ordered = frame.sort_values(["row_key", "column_key", "x_value", "method"])
...
def build_report(protocol_path: Path, output: Path) -> Path:
    ...
    for section in SECTION_ORDER:
        ...
        for figure_index, figure_key in enumerate(ordered_unique(section_frame["figure_key"]), 1):
            ...
            shapes = ", ".join(ordered_unique(frame["shape"]))
            descriptor = figure_key.replace("_", " ")
            title = f"{shapes}: {descriptor}"
            ...
            lines.extend([
                ...
                f"![{title}]({image_prefix}/{filename})", "",
                *markdown_table(["Specification", "Setting"], specification_rows(frame, populations, section)), "",
                "Null-point rejection and validity:", "",
                *markdown_table(["Vertical regime", "Horizontal regime", "Method", "Rejection", "Valid"], null_rows(frame)), "",
    ...
    lines.extend([
        f"## {section_number + (1 if runtime_lines else 0)}. Reproducibility", "",
