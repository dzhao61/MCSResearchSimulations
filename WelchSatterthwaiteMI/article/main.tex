...
\newcommand{\method}{BCW--DMI}

\title{\vspace{-1.2em}\textbf{Finite-Sample Differential Mutual Information
Inference: A Bias-Corrected Welch-Type Test and Variance Audit}}
\author{\textbf{Daniel Zhao}\\
\small Master's Research in Information Theory\\
\small Draft methods article}
\date{\small 3 August 2026}
...
finite samples. We study a deterministic bias-corrected Welch-type
differential-MI test (\method) that combines the classical first-order
correction for discrete plug-in MI, an empirical influence-function standard
error, and a Student reference. This construction is an MI extension of the
Hutcheson entropy-comparison template, not a wholly new statistical form.
...
plug-in influence variances. A post-hoc 960,000-pair diagnostic found that a
full variance-functional influence calculation reduced hard-grid error to
0.00823 and reproduced the direction on six newly generated hard populations
(0.00993 to 0.00662), but was not uniformly better across 72 regular
scenarios. The evidence supports a low-cost research direction, not an exact
Welch theorem or a universal sparse-table solution.
\end{abstract}
...
This article studies a deliberately small modification, denoted \method{}.
It first
...
The contribution is a transport, audit, and evaluation rather than the
invention of a test template. Hutcheson's entropy test already combines a
difference of Shannon functionals, delta-method variances, and a
Satterthwaite reference \citep{hutcheson1970}. The present work asks what
happens after transporting that architecture to discrete MI, adding its
specific leading bias correction, and evaluating it under the unrestricted
weak null in Eq.~\eqref{eq:null}. The degrees-of-freedom audit then identifies
why the textbook substitution is incomplete and motivates an MI-specific
variance-functional refinement.
...
Hutcheson's classical test compares two Shannon diversity values by dividing
their difference by estimated entropy variances and using Satterthwaite
degrees of freedom \citep{hutcheson1970}. Its modern implementation uses the
same broad structure as Eqs.~\eqref{eq:se} and \eqref{eq:df}, with \(n\)
rather than \(n-1\) in the component denominators. Extending it to MI is not
automatic: MI depends jointly on cell and marginal probabilities, has leading
bias \(d/(2n)\), and has a degenerate first-order influence function at
independence. Nevertheless, Hutcheson is the direct template and defines the
appropriate novelty boundary.

The idea of comparing MI across populations is also established. Mora and
Ruiz-Castillo derived asymptotic inference and pairwise comparisons for the
discrete MI segregation index in a working-paper line \citep{mora2009}; their
published account establishes the wider role and properties of the M index
\citep{mora2011}. Related segregation research shows how bias adjustments,
asymptotic tests, and bootstrap refinements can be combined for nonlinear
indices \citep{allen2015}. Hart and Giszter combined jackknife uncertainties
when comparing histogram MI estimates in neuroscience \citep{hart2010}.
...
estimates from a continuous KSG estimator \citep{martin2026}; the estimator
itself builds on controlled-bias KSG work such as \citet{holmes2019}.
...
Resampling is an essential comparator rather than an afterthought. Bootstrap
methods can improve nonlinear segregation-index inference
\citep{allen2015}, and simulation studies of Shannon diversity report strong
coverage for bootstrap-\(t\) intervals \citep{palma2022}. A multinomial
bootstrap-\(t\) comparison for differential MI is therefore required in the
...
\section{Bias-Corrected Welch-Type Test}
...
Thus, at a fixed \(T\), \(p_{\mathrm{BCW}}\) is never smaller than the normal
...
Under fixed finite alphabets, positive population cell probabilities,
independent and identically distributed observations within each group,
independent groups, sample-size proportions bounded away from zero and one,
and positive influence variances, Eq.~\eqref{eq:linear} implies that \(T\)
converges in distribution to \(N(0,1)\) under \(H_0\). Moreover,
\(\widehat\nu\to\infty\), so the Student reference converges to the same
normal limit. This is asymptotic compatibility by construction, not a
second-order accuracy result.

Classical Welch inference combines ordinary sample
...
\subsection{Post-hoc variance diagnostic}

A subsequent methodological audit fixed a third population seed before
simulation. It generated 20,000 new table pairs for each of the 12 decisive
hard populations and 10,000 pairs for each of 72 entirely new weak-null
populations, totalling 960,000 comparisons. It measured
\(\E(\widehat V)/V\), the empirical variance of
\(\widehat\Delta_{\mathrm{BC}}\), and its correlation with the estimated
variance. It also compared Eq.~\eqref{eq:df} with local-score-kurtosis degrees
of freedom, full variance-functional influence degrees of freedom, and a
diagnostic normal test using the known population variance. Because this
audit was designed after inspecting the original study, it is explicitly
post-hoc even though its new population and simulation seeds were fixed
before execution.
...
and smaller \(|\mathrm{FPR}-0.05|\) is better. Candidate methods were applied
to the same table pair in every replicate. Consequently, their rejection
changes are paired; the larger marginal Monte Carlo uncertainty of either FPR
does not describe the uncertainty of the much smaller within-replicate
difference.
...
The stress-grid result is negative at the scenario level. Although mean FPR
moved toward 0.05, the proportion inside the practical calibration band fell
from 19.23\% to 15.38\%. The correction is monotone conservative, so already
conservative \(2\times2\) settings became worse; the present method has no
validated diagnostic that identifies such cases in advance. In addition, 34
first-order-degenerate cases were explicitly returned as invalid. The
99.987\% valid rate is high, but stress-grid FPR is conditional on a
computable positive standard error.
...
\((\mathrm{MAE}_{N}-\mathrm{MAE}_{\mathrm{BCW}})\).}
\label{tab:holdout}
\small
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}lrrrrr@{}}
\toprule
Holdout stage & Populations & Replicates & Normal MAE & BCW MAE &
...
\begin{figure*}[t]
  ...
  \caption{\textbf{Accuracy and cost summary.} Left: normal and BCW--DMI MAE
...
\subsection{Variance bias and refined degrees of freedom}

The post-hoc diagnostic rejected a simple universal variance-bias
explanation. Mean estimated \(SE^2\) divided by its population first-order
value was 1.001 on the decisive hard grid and 1.066 on the fresh 72-scenario
grid, while the empirical variance of the corrected MI difference was 1.071
and 1.063 times that population value. Even the oracle-variance normal test
was not uniformly calibrated. Thus, finite-sample numerator variation and
numerator--denominator dependence matter alongside variance estimation.

For completeness, let \(V(P)=\Var_P(\ell)\), \(m_2=\E_P(\ell^2)\), and
\(\mu=\MI(P)\). The influence function of the variance functional is
\begin{align}
g_P(x,y)={}&\ell_{xy}^2-m_2
 +2\{\ell_{xy}-\E(\ell\mid X=x)\nonumber\\
&-\E(\ell\mid Y=y)+\mu\}
 -2\mu(\ell_{xy}-\mu).
\label{eq:varianceif}
\end{align}
Writing \(\tau_P^2=\Var_P\{g_P(X,Y)\}\), first-order moment matching gives
\(\nu_{V,P}=2n_PV(P)^2/\tau_P^2\). Plug-in versions for \(P\) and \(Q\) can
replace \(n_P-1\) and \(n_Q-1\) in Eq.~\eqref{eq:df} at unchanged
\(O(rc)\) order.

Table~\ref{tab:postaudit} shows why this candidate deserves a new
confirmatory study. It materially improved the deliberately liberal hard
regimes and had its largest gain at \(\alpha=0.01\). Across all fresh regular
scenarios, however, it improved 28, worsened 39, and tied five at
\(\alpha=0.05\); the simpler local-kurtosis approximation had slightly lower
mean MAE there. All candidate valid-result rates were one in this audit.

\begin{table*}[t]
\centering
\caption{\textbf{Post-hoc degrees-of-freedom diagnostic.} Entries are mean
absolute FPR errors. Variance IF uses Eq.~\eqref{eq:varianceif}; the oracle
uses known population variances and is diagnostic, not usable.}
\label{tab:postaudit}
\small
\begin{tabular}{@{}lrrrrr@{}}
\toprule
Population set (\(\alpha\)) & Normal & \(n-1\) Welch & Local kurtosis &
Variance IF & Oracle\\
\midrule
Decisive hard (0.05) & 0.01270 & 0.01169 & 0.01040 & \textbf{0.00823} & 0.00785\\
Decisive hard (0.01) & 0.00568 & 0.00506 & 0.00428 & \textbf{0.00266} & 0.00354\\
Fresh hard (0.05) & 0.00993 & 0.00912 & 0.00775 & \textbf{0.00662} & 0.00837\\
Fresh hard (0.01) & 0.00407 & 0.00360 & 0.00317 & \textbf{0.00232} & 0.00317\\
Fresh all (0.05) & 0.00463 & 0.00445 & \textbf{0.00423} & 0.00446 & 0.00756\\
\bottomrule
\end{tabular}
\end{table*}
...
that rule, and the generated metadata correctly records NO-GO. The
variance-functional result was obtained later and cannot retroactively change
that decision. It instead defines a new candidate requiring a separately
frozen protocol. This distinction preserves the original numerical result,
comparators, and report while allowing the research direction to evolve.

\section{Discussion}

\subsection{What the method adds}

\method{} establishes a transparent MI-specific baseline between normal
analytic inference and studentized permutation. It adds almost no
computational burden and provides a small conservative adjustment where the
normal reference is mildly liberal, but Hutcheson's entropy test makes clear
that this architecture is transported rather than invented. The stronger
methodological contribution is the variance audit: it explains why assigning
\(n-1\) to a nonlinear plug-in variance is unjustified and supplies a
computable \(O(rc)\) alternative through Eq.~\eqref{eq:varianceif}.
...
values, so few decisions change. The variance-functional candidate lowers
those degrees of freedom in the regimes where estimated uncertainty is most
variable. Its nonuniform broad-grid behaviour shows that this is still an
approximation to a dependent studentized statistic, not a universal pivot.
...
Hutcheson-type MI baseline with heuristic \(n-1\) component df.\\
Variance-IF candidate & \(\MI(P)=\MI(Q)\) & \(O(rc)\), deterministic &
Post-hoc MI-specific moment refinement requiring confirmation.\\
...
The original construction claim must be narrowed. Hutcheson introduced the
entropy analogue in 1970 \citep{hutcheson1970}; Mora and Ruiz-Castillo
developed asymptotic comparison for the MI segregation index
\citep{mora2009}; and published examples use Welch on repeated continuous-MI
estimates \citep{martin2026} and per-unit MI outcomes
\citep{prince2021}. Nor are the plug-in estimator, \(d/(2n)\) correction,
influence variance, studentization, or Satterthwaite formula individually new.

The defensible contribution is therefore methodological transport plus audit:
an explicit Hutcheson-type procedure for two independent discrete MI values
under the unrestricted weak null, unusually broad paired finite-sample
evaluation, and the derivation and preliminary test of variance-functional
degrees of freedom tailored to the MI uncertainty estimator. The last element
is more distinctive than the \(n-1\) baseline but remains post-hoc. Before any
publication-priority statement, the search should be repeated in
authenticated Scopus, Web of Science, MathSciNet, zbMATH, and ProQuest with
Hutcheson and diversity-index terms included.
...
samples, increase the outer permutation replicates, pre-specify
\(\alpha=0.01\), add multinomial bootstrap-\(t\), test declared versus
observed support dimensions, and include real-data demonstrations. These
additions improve external validity; simply increasing the existing null
replicate counts would add little.

\subsection{Next methodological step}

The next step is a pre-specified head-to-head study of the local-kurtosis and
full variance-functional references. Equation~\eqref{eq:varianceif} tracked
empirical component degrees of freedom more closely than \(n-1\) and produced
the stronger hard-grid result, whereas local kurtosis had slightly better
average fresh-grid MAE. Neither may replace Eq.~\eqref{eq:df} until evaluated
on another untouched population family against studentized permutation and
bootstrap-\(t\). Corrected Satterthwaite formulas proposed for general
variance synthesis may also be relevant \citep{vondavier2026}, but their
assumptions are not yet established for MI.

\section{Conclusion}

\method{} transports the Hutcheson--Satterthwaite comparison architecture to
a distinct information-theoretic problem: equality of two independent
discrete MI values under a weak null. Leading plug-in bias correction and
MI-specific influence uncertainty make the transport substantive, but the
\(n-1\) reference itself is only mildly beneficial. Studentized permutation
remained more accurate.

The audit changes the research emphasis. The estimated MI difference and its
plug-in uncertainty are dependent, the ordinary component degrees of freedom
are too large, and no single variance rescaling repairs calibration. A full
variance-functional influence calculation is deterministic, remains
\(O(rc)\), and produced materially lower hard-regime error in a new
diagnostic, especially at \(\alpha=0.01\). Its nonuniform regular-grid
behaviour demands a fresh confirmatory study. The defensible thesis is thus
not a new Welch test, but an MI-specific transport, failure analysis, and
promising moment refinement with clearly defined limits.
...
\texttt{experiments/run\_validation.py}; the post-hoc variance audit is
\texttt{experiments/audit\_variance\_bias.py}. The baseline implementation is
