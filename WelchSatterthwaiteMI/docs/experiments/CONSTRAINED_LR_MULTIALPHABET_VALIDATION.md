...
an acceptable fit. Rejection rates are conditional on valid results. When
methods have different valid sets, their conditional rejection rates do not
refer to exactly the same replicates; a low valid rate is therefore a method
failure and not evidence of good calibration.
...
These are screening estimates. At a true rejection rate near 0.05, the
approximate 95% Monte Carlo half-width is 0.014 with 1,000 replicates and 0.027
with 250 replicates. Small differences, especially for 8x8 tables, should not
be interpreted as established method differences without confirmation.
...
### 4.2 Focused confirmation

Six exact configurations were selected before a new 2,000-replicate null run.
The selection included an ordinary control, three apparent LR gains, and two
apparent LR losses from the screen.

| Shape | Regime | $N$ | Normal Wald FPR | Expanded Welch FPR | LR FPR | LR valid rate |
| --- | --- | ---: | ---: | ---: | ---: | ---: |
| $3\times3$ | Balanced | 250 | 0.0475 | 0.0390 | **0.0510** | 1.0000 |
| $3\times3$ | Ultra-skewed | 25 | 0.0870 | **0.0605** | 0.0100 | 1.0000 |
| $5\times5$ | Balanced | 50 | 0.0810 | 0.0635 | **0.0545** | 1.0000 |
| $5\times5$ | Ultra-skewed | 50 | **0.0478** | 0.0363 | 0.0210 | 0.9995 |
| $8\times8$ | Mildly skewed | 25 | 0.0965 | 0.0835 | **0.0461** | 0.9660 |
| $8\times8$ | Strongly skewed | 50 | 0.0895 | 0.0658 | **0.0506** | 0.9985 |

The focused run confirms that LR can correct substantial liberal distortion
without overcorrecting in several balanced, mild, and strongly skewed sparse
tables. It also confirms strong LR conservatism in the two ultra-skewed cases.
The ultra-skewed Wald and Expanded Welch rates are conditional on lower valid
rates, so direct method comparisons there require additional common-valid
replicate analysis.
...
### 4.4 Power curves across MI differences
...
### 4.5 Numerical validity and runtime

In the fixed-effect screen, LR converges on every 3x3 and 4x4 replicate. Its
minimum valid rate is 0.998 for 5x5 and 0.968 for 8x8. In the separate
power-curve run, the corresponding minima are 1.000, 1.000, 0.998, and 0.952
for 3x3, 4x4, 5x5, and 8x8. Every accepted fit in both experiments has an
equal-MI constraint residual below $10^{-8}$.
...
The constrained LR implementation extends successfully beyond binary tables
in the numerical sense: it handles all tested shapes, remains highly valid,
and often reduces the worst liberal errors seen in Wald. Its statistical
advantage is not universal. Wald remains competitive in ordinary 3x3--5x5
cases, while LR can overcorrect at the most extreme low-sample boundary.

The most defensible current conclusion is therefore:

> Constrained LR was numerically feasible and promising through 8x8 tables in
> this screen. It often protected against severe liberal calibration in larger
> sparse tables, but it did not uniformly improve calibration and power over
> Normal Wald.

The broad 8x8 screen uses only 250 replicates per configuration. The focused
run confirms two selected 8x8 gains with 2,000 replicates each, but it does not
confirm every 8x8 regime. The main remaining evidence gap is external
generality: each shape-regime combination currently uses one deterministic
population pair. A stronger follow-up should use several independently
constructed population pairs within each selected regime and should retain
the ultra-skewed cases where LR was conservative.
...
- [Focused confirmatory report](../../results/multialphabet_lr_confirmatory/REPORT.md)
- [Focused confirmatory results](../../results/multialphabet_lr_confirmatory/results.csv)
- [Focused confirmatory runner](../../experiments/run_multialphabet_lr_confirmatory.py)
...
completed in 2,632.5 seconds with eight worker processes. The focused
confirmation adds 12,000 independently sampled null table pairs.
