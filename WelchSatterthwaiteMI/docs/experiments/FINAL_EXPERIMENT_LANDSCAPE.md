...
point is based on 10,000 independently simulated table pairs. The figures cover
every power and robustness configuration together with its matching null
point. The finer null-only sample-size sweep, Simple Welch, and the secondary
$\alpha=0.01$ and $\alpha=0.10$ results remain available in the source results
but are omitted from this briefing.
...
Every graph in this section begins at $e=0$, where its rejection rate is the
false-positive rate, and continues through $e>0$, where its rejection rate is
power. Equal samples, unequal samples, and alternative interaction patterns
are treated as different regime groups within this same framework.

### 4.1 Equal sample sizes and primary interactions
...
#### 4.1.1 Shape 2x2: same population path
...
#### 4.1.2 Shape 2x2: different population paths
...
| Graph rows and columns | the same skewness levels and sample sizes as the preceding graph |
| Horizontal values | the same eight values of $e$ from 0 to 0.60 |
| $M$ by row | balanced 0.693147; mild 0.132829; strong 0.011134; ultra 0.002633 |

#### 4.1.3 Shape 2x3: same population path
...
#### 4.1.4 Shape 2x3: different population paths
...
#### 4.1.5 Shape 3x3: same population path
...
#### 4.1.6 Shape 3x3: different population paths
...
#### 4.1.7 Shape 3x5: same population path
...
#### 4.1.8 Shape 3x5: different population paths
...
#### 4.1.9 Shape 4x4: same population path
...
#### 4.1.10 Shape 4x4: different population paths
...
#### 4.1.11 Shape 4x8: same population path
...
#### 4.1.12 Shape 4x8: different population paths
...
#### 4.1.13 Shape 5x5: same population path
...
#### 4.1.14 Shape 5x5: different population paths
...
#### 4.1.15 Shape 8x8: same population path
...
#### 4.1.16 Shape 8x8: different population paths
...
| --- | --- |
| Shape | $3\times3$ |
| Graph rows, columns and effects | same design as the preceding 2x2 graph |
| Population relationship | different paths; equal MI at $e=0$ |
...
### 4.3 Alternative interaction patterns
...
| --- | --- |
| Shape | $3\times5$ |
| Graph rows, columns and effects | same design as the preceding 3x3 graph |
| Population relationship | different paths; equal MI at $e=0$ |
...
5. Use Sections 4.2 and 4.3 after the equal-sample graphs to see how sample
   imbalance and alternative interactions change the same rejection curves.
