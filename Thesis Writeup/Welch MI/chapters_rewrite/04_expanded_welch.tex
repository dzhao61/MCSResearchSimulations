...
test statistic from Chapter~\ref{chap:baselines}. It evaluates that statistic
against an estimated Student reference rather than the standard normal. This
chapter derives the required degrees of freedom from the sampling variability
of the complete MI variance estimator.
...
\end{equation}
The symbol $\dot\sim$ denotes a working distributional approximation. The
model matches moments; it does not assert that the
...
pointwise-MI value. The bracket accounts for changes in the pointwise-MI
values themselves, including the selected joint probability and both margins.
...
A first-order Taylor expansion of the variance functional around $P$ is
\begin{equation}
  \Vhat(P)-V(P)
  \approx
  \frac{1}{n_P}
...
\end{equation}
Equation~\eqref{eq:population-component-df} depends on relative, not
absolute, uncertainty. Since
\begin{equation}
  \frac{\Var\{\Vhat(P)\}}{V(P)^2}
  \approx \frac{\tau^2(P)}{n_PV(P)^2},
\end{equation}
the component degrees of freedom are approximately twice the reciprocal of
this relative variance. A stable estimate relative to the size of $V(P)$ has
large degrees of freedom; a relatively unstable estimate has smaller degrees
of freedom and produces a heavier-tailed final reference. This
first-order component formula assumes \(\tau^2(P)>0\). Positive \(V(P)\)
alone does not guarantee that condition: at a stationary point of the
variance functional, its first-order sensitivity can vanish even though MI
and \(V(P)\) are positive. There, a higher-order analysis of \(\Vhat(P)\)
would be needed to describe its finite-sample distribution.
...
sensitivity needs to be evaluated there. The empirical mean and variance are
\begin{align}
  \overline g_P
...
$t$ variable with $\nu$ degrees of freedom. As with Wald, only test behaviour
is evaluated; no confidence-interval coverage claim is made.

The expanded calculation does not change \(\Deltahat\), \(\SEhat\), or
\(T\). It changes only the quantile used to evaluate that statistic.

\begin{proposition}[Nested rejection on common-valid samples]
If the two calculations use the same finite statistic \(T\) and Expanded
Welch has positive finite degrees of freedom, its two-sided p-value is at
least the Normal Wald p-value. Thus, at a common fixed threshold, every
Expanded Welch rejection is also a Wald rejection on samples where both are
valid.
\end{proposition}
\begin{proof}
Write a Student variable as \(Z/\sqrt S\), with independent
\(Z\sim N(0,1)\) and \(S\sim\chi^2_\nu/\nu\). For \(c>0\),
\(f(s)=2\{1-\Phi(c\sqrt{s})\}\) has
\(f''(s)=c\phi(c\sqrt{s})(s^{-3/2}+c^2s^{-1/2})/2>0\).
Since \(\E S=1\), Jensen's inequality gives
\(\Pr(|Z/\sqrt S|>c)>\Pr(|Z|>c)\). At \(c=0\) both tails equal one.
\end{proof}
The proposition does not
classify samples on which one method is invalid. In particular, a correction
that only changes the reference cannot raise rejection where Wald is already
too conservative.
...
For Expanded Welch, the table-based sensitivity calculation first gives
\(\widehat\tau^2(P)=0.93212\) and
...
\(\widehat\nu_{\mathrm{expanded}}=59.03\), giving \(p=0.15264\). Both methods
fail to reject at the 5\% level for these tables. The example shows that
Expanded Welch changes the reference, not the observed statistic.
...
7 & Return the Student p-value, effective degrees of freedom, and validity diagnostics. \\
\bottomrule
\end{tabularx}
\end{table}
...
\section{Behaviour near independence}
\label{sec:near-independence-mechanism}

The size of the adjustment has a useful local explanation. Hold the positive
margins fixed and write \(p_{ij}=a_{ij}+\epsilon h_{ij}\), where
\(a_{ij}=p_{i+}p_{+j}\), the row and column sums of \(h\) vanish, and
\(\epsilon\) tends to zero. With \(s_{ij}=h_{ij}/a_{ij}\), a second-order
Taylor expansion gives
\begin{align}
 I(P)&=\tfrac12\epsilon^2\sum_{ij}a_{ij}s_{ij}^2+O(\epsilon^3),\\
 V(P)&=\epsilon^2\sum_{ij}a_{ij}s_{ij}^2+O(\epsilon^3)
       =2I(P)+O(\epsilon^3).
\end{align}
The row and column conditional means of the leading score
\(\epsilon s_{ij}\) are zero. In Equation~\eqref{eq:g-final}, the bracket
therefore contributes \(2\epsilon s_{ij}+O(\epsilon^2)\), whereas the
centred squared term is only \(O(\epsilon^2)\). Consequently,
\begin{equation}
 \tau^2(P)=4V(P)+O(\epsilon^3),\qquad
 \nu_P\approx\frac{n_PV(P)}{2}\approx n_PI(P).
 \label{eq:near-independent-df}
\end{equation}
This is a population component approximation. The final two-sample degree
of freedom still combines both components: for equal populations and equal
sample sizes it is approximately \(2nI(P)\). A small population component
also need not equal the median of its empirical plug-in estimate.

These relations explain why low baseline MI can produce a large Student
adjustment. They also reveal a limit to its first-order justification.
In the local Gaussian approximation to multinomial errors, the quadratic
interaction statistic has a noncentral chi-squared form with
\(d=(r-1)(c-1)\) coordinates and local noncentrality
\(\lambda\approx nV(P)\approx2nI(P)\). The last equivalence uses the local
relation \(V(P)\approx2I(P)\); it is not exact away from independence.
Retaining the linear and quadratic error terms then gives the local formulas
\begin{align}
 \E\Vhat&\approx V+\frac d n,&
 \Var(\Ihat)&\approx\frac V n+\frac d{2n^2},\\
 \Var(\Vhat)&\approx\frac{4V}{n}+\frac{2d}{n^2},&
 \nu_{\rm moments}&\approx\frac{(nV+d)^2}{2nV+d}.
 \label{eq:local-variance-mechanism}
\end{align}
Here \(\nu_{\rm moments}=2\{\E\Vhat\}^2/\Var(\Vhat)\) includes the
quadratic terms omitted by the first-order component formula. Appendix
\ref{app:mechanism} derives these expressions and tests them numerically.
They require fixed dimension, positive margins and an adequate Gaussian
approximation to the cell counts; they are not universal finite-sample
bias formulas for arbitrary MI or sparse support.

The estimated variance of MI can therefore be too large even when the
numerator's sampling variance also exceeds its first-order value. Changing
to a Student reference lowers rejection further. To neglect the quadratic
terms in this local model requires \(nV\) large relative to \(d\), not just
a large total sample size. This warns against first-order moment matching near
independence; it does not prove that small degrees of freedom can never be
useful.
...
moments of the estimated variance and do not provide an exact finite-sample
law. Moreover, \(\Ihat(P)\) and \(\Vhat(P)\) use the same observations; their
first-order covariance is generally
\(\E_P[\{\pmi_P(X,Y)-I(P)\}g_P(X,Y)]/n_P\), not zero. Moment matching
the denominator does not establish independence from the numerator or an
exact Student law. Under regular large-sample conditions with positive
...
The software first requires matching table shapes, at least two rows and two
columns, finite nonnegative integer counts, and more than one observation in
each table. Wald is then valid only when the corrected difference and standard
error are finite and $\Vhat(P)+\Vhat(Q)>10^{-14}$. Expanded Welch
additionally requires positive finite
component and combined degrees of freedom. Failed conditions are recorded as
invalid calculations; no different test is silently substituted. The
validation in Chapters~\ref{chap:design} and~\ref{chap:results} evaluates how often these conditions
