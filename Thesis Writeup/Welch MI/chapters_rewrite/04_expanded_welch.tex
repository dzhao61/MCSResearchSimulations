...
\end{equation}
Equation~\eqref{eq:population-component-df} depends on relative, rather than
...
$t$ variable with $\nu$ degrees of freedom. The corresponding working
...
\end{equation}
Here $t_{\nu,\gamma}$ is the $\gamma$ quantile of that Student distribution.
...
7 & Return the Student p-value, effective degrees of freedom, and validity diagnostics. Equation~\eqref{eq:expanded-ci} gives an interval if required. \\
\bottomrule
\end{tabularx}
\end{table}
...
\(d=(r-1)(c-1)\) coordinates and noncentrality approximately \(2nI(P)\).
...
a large total sample size. This is a warning about using first-order moment
matching near independence, rather than a proof that small degrees of
freedom can never be useful.
...
invalid calculations rather than silently replaced by a different test. The
