...
The size of the adjustment has a useful local explanation. For a distribution
close to independence, a second-order Taylor expansion gives
\begin{equation}
 V(P)\approx 2I(P),
 \qquad \tau^2(P)\approx4V(P).
 \label{eq:near-independent-moments}
\end{equation}
Substitution into the component formula then gives
\begin{equation}
 \nu_P=\frac{2n_PV(P)^2}{\tau^2(P)}\approx n_PI(P).
 \label{eq:near-independent-df}
\end{equation}
Thus low MI can produce a small effective degree of freedom and a much
heavier-tailed reference. The approximation is local: it assumes fixed table
dimension, positive probabilities and sufficiently regular cell counts. Very
near independence, quadratic sampling variation can still be important, so a
first-order Student adjustment need not improve finite-sample calibration.
