...
\end{equation}
where \(\Phi\) is the standard normal distribution function. The thesis
evaluates this test through p-values and rejection rates. It does
not claim confidence-interval coverage, which would require a separate
coverage experiment.

The normal reference is a large-sample approximation. Wald estimates the
standard error from the tables but applies no degrees-of-freedom adjustment.

\section{What the expanded method must calculate}

The shared statistic supplies the estimated effect and its first-order
standard error. The direct Hutcheson-style information-theoretic analogue
assigns component degrees of freedom \(n_P\) and \(n_Q\); a familiar
ordinary-Welch analogue uses \(n_P-1\) and \(n_Q-1\). Both fixed assignments
are retained as explanatory baselines. However, \(\Vhat(P)\) is a nonlinear
functional of a table whose margins and pointwise MI values also change with
the data. The expanded method instead estimates component degrees of freedom
from that full functional.
