...
The stored unrounded probabilities, rather than the rounded display in
...
table; only its embedding differs. This four-cell perturbation is a standard
sparse alternative in contingency-table simulation studies
\citep[Section~3.2]{berrett2021}. Here it is adapted to compare two fixed
populations: their margins are prescribed separately and the perturbation
strength is chosen to attain a specified MI rather than used directly as the
...
Wilson 95\% intervals represent Monte Carlo uncertainty at the fixed
...
There are 3,111 unique statistical configurations but 4,001 display points:
