...
Independence of the two samples allows their variance contributions to be
added, but it does not make \(T\)'s numerator independent of its estimated
standard error. The representative pilot in Chapter~\ref{chap:results} shows
that both parts can depart from their first-order approximations. Moment
matching the denominator alone therefore cannot guarantee a Student law or
correct every finite-sample error.
...
\(nI\). This explains why the reference can become very heavy-tailed at low
MI, exactly where the first-order approximation is least reliable. The result
is a useful warning, not a complete finite-sample theory.
...
return a p-value constitute a true negative. The saved validity and paired
results allow readers to distinguish fewer rejections from fewer usable
tests.
...
The alternative curves use the common nominal 0.05 threshold. They show
practical detection under the tested procedures, not power after giving each
method a simulation-specific critical value.
