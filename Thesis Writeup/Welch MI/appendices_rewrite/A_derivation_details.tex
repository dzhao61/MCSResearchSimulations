...
\end{equation}
Substitution into the first-order Taylor expansion of MI around $P$ gives
\begin{align}
  \Ihat(P)-I(P)
  &\approx
...
$V(P)/n_P$, not a variance of the numerical category labels.

\section{Mean of the variance influence function}

The influence function $g_P(x,y)$ is centred at the population level. This
can be shown without expanding its individual terms. Write the one-cell
direction as $h^{(x,y)}=\delta_{(x,y)}-P$. Its probability-weighted average is
\begin{equation}
  \sum_{x,y}p_{xy}h^{(x,y)}=P-P=0.
\end{equation}
...
\begin{align}
  ...
  &=\left.\frac{\mathrm d}{\mathrm d\varepsilon}
    V\left\{P+\varepsilon
    \sum_{x,y}p_{xy}h^{(x,y)}\right\}
    \right|_{0}\\
  &=\left.\frac{\mathrm d}{\mathrm d\varepsilon}V(P)
    \right|_{0}=0.
\end{align}
The same weighted cancellation holds after exact plug-in substitution. The
implementation nevertheless subtracts $\overline g_P$ in
Equation~\eqref{eq:tau-hat} to protect the variance calculation from
floating-point roundoff; this does not change the population definition.
...
differences on positive probability tables. For a selected cell and a small
positive step $\eta$, define
\begin{equation}
  P_{\pm\eta}=(1\mp\eta)P\pm\eta\delta_{(x,y)}.
\end{equation}
For sufficiently small $\eta$, the checks compare
\begin{align}
  \frac{I(P_\eta)-I(P_{-\eta})}{2\eta}
  &\quad\text{with}\quad \pmi_P(x,y)-I(P),\\
  \frac{V(P_\eta)-V(P_{-\eta})}{2\eta}
